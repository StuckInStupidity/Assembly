...
main:
	...
	MOV ecx, 0
loop:
	CMP eax, len                      ; instruction triggering a flag Z state (could have been subtraction for ex)
	JE end                            ; (JE, JNE, JL, JG, JLE, JGE)
	MOV ebx, [list + eax]             ; memory access by indexation (mov in ebx, the value stored at address list + eax)
	ADD ecx, ebx
